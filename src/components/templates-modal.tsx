    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent showCloseButton={false} className="rounded-2xl sm:max-w-[960px] h-[calc(100vh-170px)] !flex !flex-col bg-[#212121] border-0 text-white p-0 gap-0 overflow-hidden shadow-2xl ">
        <DialogHeader className="p-4 border-b border-[#2c2c2c] h-[64px]!">
          <DialogTitle className="font-sans text-sm font-normal translate-y-[7px]">Choose a background</DialogTitle>
        ...
        <DialogFooter className="p-4 border-t border-[#2c2c2c] flex flex-row items-center justify-between sm:justify-between w-full  h-[64px]!">
          <span className="text-[#737373] text-sm font-sans">
            Click a template to select it
          </span>
          <div className="flex items-center gap-3">
            <Button
              onClick={onClose}
              className="text-white hover:text-white font-sans rounded-[8px] h-9 px-3 border border-[#2c2c2c] hover:bg-white/5 bg-[#212121]"
            >
              Cancel
            </Button>
            <Button
              ...
              className="bg-white text-black hover:bg-white/90 font-sans rounded-[8px] h-9 px-3 disabled:opacity-50"
            >
              Use template
            </Button>
          </div>
