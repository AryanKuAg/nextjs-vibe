...
/**
 * The interactive half of a v0 turn.
 *
 * v0 can pause mid-build to ask a question, propose a plan, request access to a
 * service, or ask permission to run a tool. Without these cards the run simply
 * stops with no way to answer it, so they are the difference between a builder
...
export function TaskResolution({
  ...
  switch (task.type) {
    ...
    case "integration":
      return <IntegrationTask data={task.data} disabled={disabled} onResolve={onResolve} />;
    case "permissions":
      return (
        <PermissionsTask
          disabled={disabled}
...
function QuestionsTask({
  ...
  // Free text counts as an answer — otherwise a question whose real answer is
  // "none of these" could never be submitted.
  const canSubmit =
  ...
  const selectOption = (question: QuestionsData["questions"][number], label: string) => {
    setSelected((current) => {
      if (!question.multiSelect) return { ...current, [question.id]: [label] };
          ...
          <fieldset className="grid gap-2" disabled={disabled} key={question.id}>
            ...
            <div className="grid gap-1.5">
              {question.options.map((option) => (
                ...
                >
                  <input
                    checked={Boolean(selected[question.id]?.includes(option.label))}
                    className="mt-0.5"
                    name={question.id}
            ...
            <Textarea
              ...
              disabled={disabled}
              onChange={(event) =>
                setCustomText((current) => ({ ...current, [question.id]: event.target.value }))
              }
...
function IntegrationTask({
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-2">
        <Button disabled={disabled} onClick={() => resolve(true)} size="sm" type="button">
          I connected these
        </Button>
        <Button
          disabled={disabled}
